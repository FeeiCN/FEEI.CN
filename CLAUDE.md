...
`image` 标签在文档有配图时填写，取正文中第一张图的相对路径（相对于 `static/`）。无图文档不写 `image`，让社交分享回退到站点默认 OG 图。
...
- 带 `演讲` 徽标（frontmatter 含 `sidebar_badge: { text: '演讲' }`）的文章属于历史快照，不修改正文。需更新观点时应复制为新文件后再编辑，原文件保留不动。
...
提交信息使用中文，风格以简短祈使句为主，例如：`添加站点图标`、`重构健康文档并恢复更新时间`、`优化财富框架`。建议一次提交只聚焦一类改动。提交信息中不添加 `Co-Authored-By` 行。
