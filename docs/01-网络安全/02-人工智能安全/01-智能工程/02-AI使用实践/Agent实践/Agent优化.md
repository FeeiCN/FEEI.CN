...
description: Agent 优化不是反复手调 Prompt，而是把评测、真实反馈、错误归因和 Skill/Harness 更新组成可审计的持续改进闭环。
...
Agent 上线后的主要问题通常不再是“能不能完成一次任务”，而是**怎样让同类任务越做越好，并把人的纠正沉淀成系统能力**。只靠人工不断修改 Prompt、补充 `AGENTS.md` 或在每次会话里重新提醒，能够解决局部问题，却很难形成复利：会话结束后，很多高质量反馈也随之消失。

更可持续的做法，是把 Agent 优化做成一个受控闭环：
...
[Loop 工程](/agent-loop-engineering)解决的是**一次任务怎样根据反馈继续执行直到收敛**；Agent 优化解决的是**多次任务之间怎样根据历史反馈改进系统本身**。
...
当同一种人工纠正越来越少，人主要处理新的目标、例外和标准变化时，Agent 优化才真正产生复利。
