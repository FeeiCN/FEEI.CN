...
Agent 能力可以沿着“思考—规划—执行—收敛—复用—优化”逐层完善：
...
| **执行循环** | 怎样持续执行直到完成 | 根据测试和外部状态继续迭代，并由停止条件决定何时退出 |
| **Skill** | 怎样复用已经验证过的工作方法 | 把“复现—最小修改—回归测试—检查 diff”沉淀为可测试流程 |
| **持续优化** | 怎样让后续同类任务越做越好 | 汇总真实反馈和 Eval，更新 Skill、Prompt、Harness、Tool 与评测集 |

这些能力可以组合使用，也可以只取当前任务需要的部分。一个只读排障任务可能停在 ReAct；准备修改真实仓库时，再增加 Harness。
...
- **需要持续执行直到任务完成**：阅读 [Agent 执行循环](/agent-loop-engineering)，用外部反馈、停止条件和预算让单次任务收敛。
- **需要让后续同类任务根据真实反馈持续变好**：阅读 [Agent 持续优化](/agent-optimization)，把 Eval、人类反馈、错误归因和 Skill/Harness 更新组成跨任务的改进闭环。
