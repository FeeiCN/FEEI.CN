...
**安全介入点一直在向前移动：从上线后修复，到上线前审计，到设计阶段评审，最终将移到代码生成的那一刻。** 当 AI 生成代码时同步完成安全检查，"写出不安全代码"这件事本身就可以被拦截，而不是等到后续环节发现再修复。

这意味着安全能力需要嵌入代码生成工具本身，而不只是在流水线末端加一道扫描。从"检测和修复"到"生成即合规"，是安全左移逻辑的自然终点。
