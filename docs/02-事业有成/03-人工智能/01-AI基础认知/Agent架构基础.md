...
## 数据 Agent：当 Agent 遇到结构化数据

上面讨论的 Agent 主要面向编程、消息、本地系统等任务。**当 Agent 接入数仓、试图做自助式业务分析时，会遇到与编程 Agent 截然不同的设计挑战。** 编程 Agent 解决"如何让代码工作"，数据 Agent 解决"如何让数字有意义"。

### 数据不是软件

**编程和数据是两种本质不同的任务。** 编程是**开放解空间**——一个问题可能有多种合理实现，奖励模型的创造力；文档和测试提供了对抗幻觉的天然护栏——写错了，测试会失败；写得不好，编译器会报错。数据分析是**单一正确答案**——某个指标在某个时间点只有一种正确计算方式；没有确定性的方法去证明这个答案是对的。这就是为什么"让 LLM 直接对数仓做 SQL"会制造虚假精确感——它能生成 SQL，但生成正确 SQL 所需的知识分散在数据模型、业务定义、schema 历史变更里。

### 三大失败模式

**绝大多数不准确回答来自这三种失败：**

**概念 ↔ 实体模糊性**：在数据模型里有数百个可行选项（来自可能多达数百万个字段），Agent 没法挑出最能回答用户问题的正确字段。比如"统计活跃用户数"——哪些行为算"活跃"？要不要排除欺诈账号？用多长的回看窗口？这些"业务定义"藏在分析师脑子里。

**数据陈旧**：数据源、业务定义、schema 时刻在变。资产和 Agent 的知识会过时，开始返回那种"看起来对但微妙错误"的答案。**陈旧不是不准确，是滞后**——更危险，因为检查不出来。

**检索失败**：正确的信息也许就在数据模型里、也做了恰当的标注，但搜索空间太大，Agent 就是找不到它。和 RAG 检索失败是同类问题，但在数据领域放大——字段级别而非段落级别，关系更复杂。

### 三大防御层

针对三种失败模式，需要**三类对应的工程层**：

**数据地基 + 真相来源**（应对实体模糊性）：把可信实体的空间收缩，直到只剩一个受治理的答案。维度建模、左移测试、对关键管道做新鲜度和完整性检查这些标准数据工程实践依然适用。**真正改变的是最终用户**——不再是数据专家，而是代表用户行事的 Agent，**而这些用户对数据专业程度参差不齐**。这要求结果不能依赖用户验证底层正确性（他们做不到），而要在数据模型层就消除歧义。

**维护 + 验证流程**（应对数据陈旧）：定义规范的仓库是天然的第一道防线（强制更新），叠加定期审计和自动化校验。**数据领域的"测试"不是单元测试，而是新鲜度/一致性/完整性检查**——发现指标是否在最新窗口、上下游是否一致、是否完整。

**Skills**（应对检索失败）：在 02-AI使用实践/AI技能.md 详尽讨论的 Skill 概念，在这里有专门用途——Skills 确保 Agent 可靠地找到并正确使用那个答案。一个数据查询 Skill 包含：数据源定位、字段语义解释、过滤条件、聚合规则、验证步骤。

### 可量化的成功基准

**以 Anthropic 自身为例：业务分析查询中约 95% 由 Claude 自动完成，整体准确率约 95%。** 把机械、重复的查询交给 Claude，数据科学团队就能专注因果建模、预测和机器学习等更具战略性的工作。**这两个 95% 是 Agent 在数据领域"达到可用"的基线参考**——不是 100%，但足以让 5% 的人工复核成为可行的工作模式。

数据 Agent 的设计原则可以提炼为：**实体模糊性靠"把治理前移"消除，陈旧靠"持续验证"对抗，检索靠"Skills 把搜索知识外化"覆盖**。这三层缺一不可，单独解决任何一层都不够。
