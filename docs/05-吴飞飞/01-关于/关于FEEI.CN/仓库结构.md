...
- `docs/`：站点主体内容，按编号分组为 `01-网络安全/`、`02-人生系统/`、`05-吴飞飞/`；人工智能相关内容统一位于 `docs/01-网络安全/02-人工智能安全/`，其中 `01-智能工程/` 是 AI 安全的基础层。人生系统下包含健康幸福、事业有成、财务自由、人生丰富四个目标，其中带 `SKILL` 徽标的文档是给 AI 协作助手使用的工作规则文件，统一放在 `关于FEEI.CN/` 子目录中。
- `docs/01-网络安全/02-人工智能安全/`：按 `01-智能工程/`、`02-AI系统安全/`、`03-AI赋能安全/`、`04-AI滥用防御/` 组织内容；通用 AI 原理、使用实践、组织转型和项目实践放在智能工程基础层，三条安全主线通过链接交叉引用。
- `blog/`：博客文章。
...
- `npm run check:docs`：检查工作区新增或修改的 Markdown/MDX；错误会阻止继续发布。
- `npm run audit:docs`：审计全量文档；存量迁移项以 warning 输出，不因历史债务失败。
- `node scripts/test_ai_navigation.mjs`：检查人工智能安全与智能工程目录、导航和相关 Markdown 文件链接；通过 `AI_MIGRATION_BASE` 指定完整基线提交 SHA，可额外验证迁移前后的文章、slug、首次发布日期与演讲快照。
- `npm run clear`：清理 Docusaurus 缓存，适用于路由或主题异常。
- Node.js `>=20`，以 `package.json` 为准。

修改文档后先运行 `npm run check:docs`。仅在 `git push` 前必须执行 build 验证：
...
- 新增 JSON 等结构化数据 → 可放到 `static/data/` 下对应目录，组件用 `/data/...` 路径引用。
