...
**客观数据**——投资持仓、iOS 健康数据、运动记录、睡眠数据……这类输入有明确的数据源和固定结构，用脚本自动同步比手动 Issue 更合适。以投资数据为例：脚本每天自动拉取证券账号持仓与历史资产，写入 MDX 文件，页面通过 ECharts 渲染资产走势、持仓分布、盈亏明细。健康数据也是同一类流程：`sync-health` 先同步仓库，再调用 `export-health-year` 通过 health-auto-export MCP 读取 Apple Health 年度数据，写入 `static/health/health_data_<year>.json`，随后更新 FEEI.CN 运行状态页，提交健康数据和状态页并推送。页面本身只负责读取结构化 JSON 并渲染图表，数据全程不需要手动维护，变动会在下一次脚本运行后自动反映在站点中。
