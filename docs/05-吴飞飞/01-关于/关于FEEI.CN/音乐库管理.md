...
音乐统一通过 `src/theme/Root.tsx` 挂载的全站悬浮播放器使用。播放内核是 `src/components/GlobalMusicPlayer/Client.tsx`，搜索、歌单与歌手选择面板是同目录的 `Controls.tsx`，数据源仍为 `playlist.ts`。不再维护独立音乐库页面；旧 `/my-playlist` 地址转到首页并打开选歌面板，不自动播放。站内可用 `?music=open` 在当前页面打开选歌面板。
...
`playlist.ts` 内三个分组，对应悬浮播放器里的基础歌单：
...
- `primaryArtistOf` 工具函数会取 `&` 前的歌手做归一，所以即使同一首歌的合作版本分别注册（`周杰伦` 和 `周杰伦 & 梁心颐`），歌手选择器不会重复建条目
...
4. 本地 `npm run start` 在任意页面点击右下角音乐按钮，验证搜索、歌单、歌手、播放、歌词、跨页面连续播放，以及 `/my-playlist` 兼容入口；仅打开面板或切换筛选不应自动播放
5. 提交并按 `仓库结构` 中的发布流程处理
