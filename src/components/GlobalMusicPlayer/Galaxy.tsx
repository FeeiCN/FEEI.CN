...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      const t = time * 0.001;
      starSpeedAcc = t * starSpeed / 10;
      program.uniforms.uTime.value = t;
      program.uniforms.uStarSpeed.value = starSpeedAcc;

      const easing = 0.05;
      smoothMouse.x += (mousePos.x - smoothMouse.x) * easing;
      smoothMouse.y += (mousePos.y - smoothMouse.y) * easing;
      smoothMouseActive += (isMouseActive - smoothMouseActive) * easing;
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      // only treat as active when cursor is over the container
      if (x >= 0 && x <= 1 && y >= 0 && y <= 1) {
        mousePos.x = x;
        mousePos.y = y;
        isMouseActive = 1;
    ...
    const onMouseLeave = () => { isMouseActive = 0; };

    if (mouseInteraction) {
      // listen on document so pointer-events:none on parent doesn't block us
