...
export type CC = {axis: string; label: string; split: string};
export type HealthDeviationLevel = 'notice' | 'warning';
export type HealthDeviation = {
  level: HealthDeviationLevel;
  tooltip: string;
};
export type DeviationPoint = {date: string; value: number};
export type DeviationSeries = {name: string; points: DeviationPoint[]};

export const HEALTH_CHART_COLORS = ['#16a34a', '#f97316', '#ef4444', '#0d9488', '#db2777', '#eab308', '#64748b'];
export const WEEKDAYS = ['日', '一', '二', '三', '四', '五', '六'];
export const MIN_BASELINE_SAMPLES = 4;
export const HEALTH_FETCH_CONCURRENCY = 3;
export const ECHARTS_RENDERER_OPTIONS = {renderer: 'canvas' as const, useDirtyRect: false};
export const HEALTH_MONTH_CACHE_LIMIT = 12;
export const CURRENT_MONTH_CACHE_TTL_MS = 60_000;
export const healthMonthCache = new Map<string, {data: HealthData; cachedAt: number}>();
export const healthMonthRequestCache = new Map<string, Promise<HealthData | null>>();
export let healthHistoryCache: HealthData | null = null;
export let healthHistoryRequestCache: Promise<HealthData | null> | null = null;
export let sleepScoreCache: HealthData['sleep_score'] | null = null;
export let sleepScoreRequestCache: Promise<HealthData['sleep_score']> | null = null;
export const sleepScoreDetailCache = new Map<string, SleepScoreDetailPayload>();
export const sleepScoreDetailRequests = new Map<string, Promise<SleepScoreDetailPayload | null>>();

export type HealthHistoryPayload = {
  data?: HealthData;
};

export type SleepScoreHistoryPayload = {
  scores?: HealthData['sleep_score'];
};

export type SleepScoreDetailPayload = {
  score?: number;
...
export function loadSleepScoreDetail(date: string): Promise<SleepScoreDetailPayload | null> {
...
export function sleepScoreTooltip(date: string, total: number, values: Map<string, number>, detail?: SleepScoreDetailPayload | null): string {
...
export function loadSleepScoreHistory(): Promise<HealthData['sleep_score']> {
...
export function readCachedHealthMonth(key: string, currentKey: string): HealthData | null {
...
export function cacheHealthMonth(key: string, data: HealthData) {
  healthMonthCache.delete(key);
...
export function fetchHealthMonth(key: string, currentKey: string): Promise<HealthData | null> {
...
export function cc(isDark: boolean): CC {
  return {
...
export function xCat(c: CC, data: [string, number][], interval?: number) {
  const n = data.length;
  const isLong = n > 400;
...
export function xAxisFromDates(c: CC, dates: string[], interval?: number) {
...
export function formatMonthDayWeekday(value: string | number) {
  const raw = typeof value === 'number' ? new Date(value).toISOString().slice(0, 10) : value;
...
export function yVal(c: CC, opts: {unit?: string; min?: number; max?: number; right?: boolean; fmt?: (v: number) => string}) {
  return {
    type: 'value',
    name: opts.unit ?? '',
...
export function ml(color: string, yVal_: number, label: string) {
  return {
...
export function smoothLine(name: string, data: number[], color: string, yIdx = 0) {
  return {name, type: 'line', yAxisIndex: yIdx, data, smooth: false, symbol: 'none', lineStyle: {color, width: 1.5}};
}

export function areaLine(name: string, data: number[], color: string, yIdx = 0) {
  return {
...
export function tooltipFmt(p: {seriesName: string; value: number; axisValue?: string}[]) {
  return `${p[0]?.axisValue ?? ''}<br/>${p.map((x) => `${x.seriesName}：${x.value}`).join('<br/>')}`;
}

export function rolling7(data: [string, number][]): number[] {
  return data.map((_, i) => {
...
export function selectedDateOrToday(selectedDate?: string): string {
  return isValidDateKey(selectedDate) ? selectedDate : dateKey(new Date());
}

export function cappedYearEnd(year: number, selectedDate?: string): string {
...
export function collectHealthDates(data: HealthData): string[] {
...
export function hasHealthData(data: HealthData): boolean {
  return (Object.keys(EMPTY) as Array<keyof HealthData>).some(
    (key) => key !== 'lastUpdated' && (data[key] as unknown[]).length > 0,
  );
}

export function getScopeAxisDates(scope: TimeScope, data: HealthData, selectedDate?: string): string[] {
...
export function xSparse(c: CC, data: [string, number][]) {
  const n = data.length;
...
export function dynGoal(arr: [string, number][], pct: number, minCount = 5): number | null {
...
export function dynRange(arr: [string, number][], pad = 2, conv?: (v: number) => number) {
...
export function base(isDark: boolean) {
  return {backgroundColor: 'transparent', color: HEALTH_CHART_COLORS, grid: chartGrid(16)};
}

export function chartGrid(right = 16, left = 12, top = 32, bottom = 28) {
  return {top, right, bottom, left, containLabel: true};
}

export function dailyBarWidth(count: number): number {
  if (count <= 14) return 8;
  if (count <= 30) return 6;
  return 4;
}

export function dualAxisGrid(isMobile: boolean) {
  return chartGrid(isMobile ? 16 : 40);
}

export function topLegend(c: CC, data: string[]) {
  return {data, top: 2, left: 'center', textStyle: {fontSize: 11, color: c.label}, itemHeight: 8};
}

export function scrollLegend(c: CC, data: string[]) {
...
export function baseTip() {
  return {tooltip: {trigger: 'axis', formatter: tooltipFmt}};
}

// ── chart option builders ────────────────────────────────────────────────────

export function stepsDistanceOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
...
export function exerciseOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function energyOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
  return {
    ...base(isDark),
    grid: dualAxisGrid(isMobile),
...
export function flightsOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function standOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
...
export function sleepOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function sleepScoreOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function wristTempOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function heartOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
...
export function cardioRecoveryOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
  return {
    ...base(isDark),
...
export function respSpo2Opt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
...
export function bodyOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
...
export function walkQualityOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
  return {
    ...base(isDark),
...
export function gaitOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
  return {
    ...base(isDark),
    grid: dualAxisGrid(isMobile),
...
export function stairOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
  return {
    ...base(isDark),
    grid: dualAxisGrid(isMobile),
...
export function sixMinWalkOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
  return {
    ...base(isDark),
    grid: chartGrid(16, 40),
...
export function audioOpt(isDark: boolean, D: HealthData, isMobile = false) {
  const c = cc(isDark);
  return {
    ...base(isDark),
    grid: dualAxisGrid(isMobile),
...
export function daylightOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function mindfulOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
  return {
    ...base(isDark),
    grid: chartGrid(16, 40),
...
export function handwashOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
  const goal = 20;
  return {
    ...base(isDark), ...baseTip(),
...
export function effortOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
  return {
    ...base(isDark), ...baseTip(),
...
export function vo2Opt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
  return {
    ...base(isDark),
    grid: chartGrid(16, 40),
...
export function workoutTimelineOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function workoutTypeOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function medDailyCombinedOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export const VALENCE_COLOR: Record<string, string> = {
...
export const KIND_ZH: Record<string, string> = {daily: '日常情绪', emotion: '即时感受'};
...
export const ASSOC_ZH: Record<string, string> = {
  community: '社区',
  current_events: '时事',
  currentEvents: '时事',
  dating: '约会',
...
export function translateList(str: string, map: Record<string, string>) {
  return str.split(',').map((s) => map[s.trim()] ?? s.trim()).join('、');
}

export function stateOfMindOpt(isDark: boolean, D: HealthData) {
  const c = cc(isDark);
...
export function addWeekends(option: object, isDark: boolean): object {
...
export function addSelectedDateHighlight(option: object, selectedDate: string | undefined, isDark: boolean): object {
...
export type TodayPoint<X, Y> = {date: string; coord: [X, Y]; color?: string};
...
export function colorValue(raw: unknown): string | null {
  return typeof raw === 'string' && raw.startsWith('#') ? raw : null;
}

export function objectColor(raw: unknown): string | null {
...
export function seriesColor(series: ChartSeries): string {
  return objectColor(series) ?? '#64748b';
}

export function latestCategoryPoint(series: ChartSeries, dates: string[]): TodayPoint<string, number> | null {
...
export function latestValueCategoryPoint(series: ChartSeries, dates: string[]): TodayPoint<number, string> | null {
  const data = series.data ?? [];
  for (let i = Math.min(data.length, dates.length) - 1; i >= 0; i--) {
    const value = numericValue(data[i]);
...
export function latestTimePoint(series: ChartSeries): TodayPoint<string, number> | null {
  const data = series.data ?? [];
...
export function latestCalendarPoint(series: ChartSeries): TodayPoint<string, number> | null {
  const data = series.data ?? [];
  for (let i = data.length - 1; i >= 0; i--) {
    const raw = data[i];
    const value = raw && typeof raw === 'object' && 'value' in raw ? (raw as {value?: unknown}).value : raw;
...
export function withTodayMark(series: ChartSeries, coord: unknown, color?: string): ChartSeries {
...
export function addTodayLatestHighlight(option: object, isDark: boolean): object {
  const opt = option as Record<string, unknown>;
...
export function mapAxis(axis: unknown, c: CC) {
...
export function mapSeries(series: unknown) {
  const normalize = (raw: unknown) => {
    const obj = asRecord(raw);
    const type = obj.type;
...
export function legendName(item: unknown): string | null {
  if (typeof item === 'string') return item;
  const obj = asRecord(item);
  return typeof obj.name === 'string' ? obj.name : null;
}

export function seriesValue(raw: unknown, series: ChartSeries): number | null {
  const value = raw && typeof raw === 'object' && 'value' in raw ? (raw as {value?: unknown}).value : raw;
...
export function orderLegendBySeriesTotal(legend: Record<string, unknown>, series: unknown): Record<string, unknown> {
...
export function isDecorativeSeries(series: Record<string, unknown>): boolean {
...
export function hasFiniteChartPoint(raw: unknown): boolean {
...
export function optionHasData(option: object): boolean {
...
export function applyHealthChartStyle(option: object, isDark: boolean): object {
  const opt = option as Record<string, unknown>;
  const c = cc(isDark);
...
export function remapIndexedSeriesData(data: unknown[], oldDates: string[], axisDates: string[]): unknown[] {
...
export function embeddedDate(raw: unknown): string | null {
  const value = raw && typeof raw === 'object' && 'value' in raw ? (raw as {value?: unknown}).value : raw;
  return Array.isArray(value) && isDateString(value[0]) ? value[0] : null;
}

export function remapEmbeddedDateSeriesData(data: unknown[], axisDates: string[]): unknown[] {
...
export function alignDateCategoryAxes(option: object, axisDates: string[]): object {
...
export function alignCalendarRange(option: object, axisDates: string[]): object {
...
export function chartHeight(label: string, isMobile: boolean) {
...
export function dateFromValue(raw: unknown): string | null {
  if (typeof raw === 'string' && raw.length >= 10 && isDateString(raw.slice(0, 10))) return raw.slice(0, 10);
  return null;
}

export function seriesDisplayName(series: ChartSeries, index: number): string {
  return typeof series.name === 'string' && series.name ? series.name : `序列 ${index + 1}`;
}

export function getAxisData(axis: unknown, index = 0): unknown[] {
  const axisObj = Array.isArray(axis) ? axis[index] : axis;
  const data = asRecord(axisObj).data;
  return Array.isArray(data) ? data : [];
}

export function numericAt(value: unknown, index: number): number | null {
  return Array.isArray(value) ? numericValue(value[index]) : numericValue(value);
}

export function extractChartSeries(option: object): DeviationSeries[] {
  const opt = option as Record<string, unknown>;
...
export function buildSeriesDeviation(series: DeviationSeries, selectedDate: string): HealthDeviation | null {
...
export function computeOptionDeviation(option: object, selectedDate: string | undefined): HealthDeviation | null {
...
export type OptionFn = (isDark: boolean, D: HealthData, isMobile?: boolean) => object;

export const SECTIONS: Array<{title: string; charts: Array<{label: string; opt: OptionFn}>}> = [
  {title: '体重', charts: [
...
export const HEALTH_CHART_METRIC_KEYS: Record<string, string[]> = {
  '体重 & 体脂率': ['weight', 'fat'],
  '每日步数 & 距离': ['steps'],
  '运动时长': ['exercise'],
  '爬楼层数': ['flights'],
...
export function slugifyChartLabel(label: string): string {
  return label
    .toLowerCase()
    .replace(/&/g, 'and')
...
export const HEALTH_CHART_REGISTRY = new Map(SECTIONS.flatMap((section) => section.charts.map((chart) => [
...
export function mergeData(all: Record<string, HealthData>, keys: string[]): HealthData {
...
export function getVisibleHealthYears(availableMonths: Record<number, number[]>, selectedDate?: string): number[] {
...
export function monthKeysThroughDate(year: number, months: number[], selectedDate?: string): string[] {
  const cap = selectedDateOrToday(selectedDate);
...
export function getTargetMonthKeys(scope: TimeScope, availableMonths: Record<number, number[]>, selectedDate?: string): string[] {
  if (scope.mode === 'period') {
...
export function filterDataByDateRange(data: HealthData, start: string, end: string): HealthData {
  const result = {} as Record<string, unknown>;
  for (const key of Object.keys(EMPTY) as Array<keyof HealthData>) {
    if (key === 'lastUpdated') continue;
...
export async function fetchHealthMonths(keys: string[], signal: AbortSignal): Promise<Record<string, HealthData>> {
...
export function retainTargetMonths(
...
export function extractDateFromChartEvent(params: unknown): string | null {
...
export function normalizeAxisList(axis: unknown): Array<Record<string, unknown>> {
  if (!axis) return [];
  return Array.isArray(axis) ? axis as Array<Record<string, unknown>> : [axis as Record<string, unknown>];
}

export function extractDateFromChartOption(option: Record<string, unknown>, params: unknown): string | null {
...
export function pickAxisDate(option: Record<string, unknown>, axisKey: 'xAxis' | 'yAxis', axisIndex: number, rawIndex: unknown): string | null {
...
export const zrenderClickHandlers = new WeakMap<ChartLike, (params: unknown) => void>();

export function extractDateFromZrenderClick(chart: ChartLike, event: unknown): string | null {
