...
const BatteryIcon = forwardRef<AnimatedIconHandle, AnimatedIconProps>(
  ({size = 24, color = 'currentColor', strokeWidth = 2.5, className = ''}, ref) => {
    const [scope, animate] = useAnimate();

    const start = async () => {
      await animate('.battery-bolt', {pathLength: 0, opacity: 0}, {duration: 0});
      animate(
        '.battery-bolt',
        {
          pathLength: [0, 1],
    ...
    const stop = () => {
      animate('.battery-bolt', {pathLength: 1, opacity: 1}, {duration: 0.2});
    };
